			...
			<div class="col-inner">
				<div class="box <?php echo esc_attr($classes_box); ?> box-blog-post has-hover">
					...
					<div class="box-text <?php echo esc_attr($classes_text); ?>" <?php echo get_shortcode_inline_css($css_args); ?>>
						<div class="box-text-inner blog-post-inner custom-title-blog-post">

							<?php do_action('flatsome_blog_post_before'); ?>

							<?php if ($show_category !== 'false') { ?>
								<p class="cat-label <?php if ($show_category == 'label')
									echo 'tag-label'; ?> is-xxsmall op-7 uppercase">
									<?php
							...
							<?php } ?>
							<h5
								class="post-title is-<?php echo esc_attr($title_size); ?> <?php echo esc_attr($title_style); ?>">
								<a href="<?php the_permalink() ?>" class="plain <?php if (is_category() || is_search()) echo 'stretched-link'; ?>"><?php the_title(); ?></a>
							</h5>
							<?php if ((!has_post_thumbnail() && $show_date !== 'false') || $show_date == 'text') { ?>
								<div class="post-meta is-small op-8 custom-time-post">
									<?php echo '<div class="day">' . get_the_time('d', get_the_ID()) . '</div><div class="month"> T' . get_the_time('m', get_the_ID()) . '</div>'; ?>
								</div>
							<?php } ?>
							<?php if (is_category() || is_search()) { ?>
								<div class="custom-category-blog-date"
									style="color: #cc0000; margin-bottom: 15px; font-weight: 400; display: flex; align-items: center; gap: 8px;">
					...
					<?php if (has_post_thumbnail() && ($show_date == 'badge' || $show_date == 'true')) { ?>
						...
						<div class="badge absolute top post-date badge-<?php echo esc_attr($badge_style); ?>">
							<div class="badge-inner">
								...
								<span class="post-date-month is-xsmall"><?php echo 'T' . get_the_time('m', get_the_ID()); ?></span>
							</div>
						</div>
					<?php } ?>
				</div>
